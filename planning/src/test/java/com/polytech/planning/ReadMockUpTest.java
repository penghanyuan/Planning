...
public class ReadMockUpTest {
	...
	public static void init() {
		...
		OriginalCourse course = new OriginalCourse("Conception et utilisation de bases de donnée", 16.0, 28.0, 20.0, 0.0, "C.Tacquard 16hCM, 20hTD x3gr + Mundus, 16hTP x3gr + Mundus  ; M. Martineau 12hTP x3gr + Mundus",true);
	...
	public void readTeachingUnits() {
		...
		List<OriginalCourse> listCourses = readMockUp.getTeachingUnits().get("Conception et utilisation de bases de donnees");
		
		System.out.println(readMockUp.getTeachingUnits().toString());
		Assert.assertTrue(readMockUp.getTeachingUnits().size() == 7);
		
		Assert.assertTrue(listCoursesExpected.get(0).equals(listCourses.get(0)));
	}
}
